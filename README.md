...
这个命令的本质是把改文件夹下的 `gemm.cpp` 文件和访问密钥发送到我们的服务器上，我们会在后台运行你的代码，然后给你一个分数。
...
编译需要安装 `g++>=7.1`，通常情况下，WSL2-Ubuntu 和我们提供的服务器环境下的 `g++` 都是足够新的。
...
> 如果你不了解这句话的含义，请把这句话和 `Makefile` 文件一起扔给 AI。
...
我们在 [缓存和寄存器性能](#缓存和寄存器性能) 中提到了如何优化矩阵乘法的寄存器性能，现在你需要再考虑上 cache，基于我们提供的框架，将这些优化实现进 `gemm.cpp` 里，目前 `gemm.cpp` 只提供了最朴素的算法用作演示。
...
性能报告会指出你实现的三个 gemm 算法的 $miss_{cache}$，$miss_{reg}$ 和 $latency$，我们会根据这个 $latency$ 来计算得分：

<!-- TODO: 得分公式 -->

其中不同 case 得分最后的加权权重为
...
<!-- 执行下面指令，会把 csim.c 和 gemm.cpp 打包成一个 `handin.tar`，请上传到 obe 中。

```bash
make handin
``` -->

<!-- TODO: 怎么提交，Github or OBE -->
